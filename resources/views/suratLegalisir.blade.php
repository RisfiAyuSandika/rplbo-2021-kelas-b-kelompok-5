...
{{--                            <h3>Data Surat Masuk <button class="btn btn-success" style="float: right" onclick="tambah()"><i class="fa fa-plus-square"></i> Tambah</button></h3>--}}
                            <h3>Data Surat Legalisir</h3>
    ...
    <div class="modal fade text-left" id="modaltambah" tabindex="-1" role="dialog"
        ...
        <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable modal-lg"
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="myModalLabel17">
                        @if(\Illuminate\Support\Facades\Session::get('pengguna')->peran == 'resepsionis')
                            Terima Pengajuan
                        @else
                            Lihat Data Pengajuan Legalisir
                        @endif
                    </h4>
                ...
                @if(\Illuminate\Support\Facades\Session::get('pengguna')->peran == 'resepsionis')
                    <form action="/suratlegalisir/terima" method="POST" enctype="multipart/form-data">
                        @elseif(\Illuminate\Support\Facades\Session::get('pengguna')->peran == 'stafTu')
                            <form action="/suratlegalisir/unggah" method="POST" enctype="multipart/form-data">
                                @endif
                                @csrf
                                <div class="modal-body">
                                    ...
                                    <div class="form-group row align-items-center">
                                        ...
                                        <div class="col-lg-10 col-9">
                                            <input type="text" class="form-control" id="tahuntamat" readonly>
                                        </div>
                                    </div>
                                    <br>
                                    @if(\Illuminate\Support\Facades\Session::get('pengguna')->peran == 'resepsionis')
...
@push('js')
    ...
    <script>
        ...
        function tambah(data){
            $('#id_pengajuan').val('PL-' + data.id)
            $('#idnya').val(data.id)
            $('#nama').val(data.nama)
            $('#nisn').val(data.nisn)
            $('#alamat').val(data.alamat)
            $('#tahuntamat').val(data.tahun_tamat)
